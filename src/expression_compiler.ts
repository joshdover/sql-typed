import { Expression, ComposedExpression, TableAttribute, ColumnExpression, ComparisonExpression, NotExpression, ComposedOp, ColumnOp, Column, Columns } from "./types";
...
const buildExpression = (
  ...
  if (isColumnExpression(expression)) {
    if (isComparisonExpression(expression)) {
      ...
      return [
        `${getColumnName(columns, expression.column)} ${stringOp} ${nextValues.valueIdx}`,
        nextValues.values
      ];
    } else {
      switch (expression.op) {
        case ColumnOp.IsNull:
          return [`${getColumnName(columns, expression.column)} IS NULL`, values];
        default:
          throw new Error(
  ...
  if (isNotExpression(expression)) {
    ...
    if (
      ...
      return [
        `${getColumnName(columns, expression.expression.column)} NOT LIKE ${nextValues.valueIdx}`,
        nextValues.values
      ];
    } else {
      let innerExp: string;
...
export const compileExpressions = (columns: Columns<any>, expressions: ReadonlyArray<Expression>, values: any[] = []): { expression: string, values: any[] } => {
  const internalExpressions = [...expressions];

  const rootExpression =
    internalExpressions.length > 1 ? joinExpressions(internalExpressions) : internalExpressions[0];

  const [expression, nextValues] = buildExpression(columns, rootExpression, values);
