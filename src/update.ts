import { TableAttributes, Update, Query, Columns, Expression, ColumnsValues, Table, Transaction, TableAttribute } from "./types";
...
export class UpdateImpl<T extends TableAttributes> implements Update<T> {
  ...
  public set<C extends keyof T>(values: ColumnsValues<T>): Update<T> {
    return new UpdateImpl(this.table, this.expressions, { ...this.columnsToSet, ...values }); 
  }

  public readonly where = (expression?: Expression) => {
    return expression === undefined
      ? this
      : new UpdateImpl(this.table, [...this.expressions, expression], this.columnsToSet);
  }
  ...
  public readonly compile = () => {
    ...
    let values: any[] = [];
    const setClauses = Object.keys(this.columnsToSet).map((column: keyof T) => {
    ...
    if (this.expressions.length > 0) {
      const compiledExpr = compileExpressions(this.table.columns, this.expressions, values);
